...
export interface DiffResult {
  ...
  astStructureDebug: {
    oldSignatureCount: number;              // 代码A的签名总数
    newSignatureCount: number;              // 代码B的签名总数
    matchedCount: number;                   // 匹配的签名数
    totalCount: number;                     // 签名总数（取各签名max的和）
  };
...
interface FunctionInfo {
  name: string;
  line: number;
  signature: string;  // AST 结构签名（忽略变量名，只保留结构）
  bodySize: number;   // 函数体 AST 节点数（代表复杂度）
}
...
interface ClassInfo {
  ...
  properties: string[];
  line: number;
  signature: string;  // 类的 AST 结构签名
  bodySize: number;   // 类体 AST 节点数
}
...
export class ASTDiffAnalyzer {
  ...
  public analyzeDiff(oldCode: string, newCode: string): DiffResult {
    ...
    let astStructureDebug = { oldSignatureCount: 0, newSignatureCount: 0, matchedCount: 0, totalCount: 0 };
    
    try {
      // 步骤2.0: 代码标准化预处理（移除 console.log、CSS 值归一化、变量名归一化）
      const normalizedOld = normalizeCode(oldCode);
      const normalizedNew = normalizeCode(newCode);

      // 步骤2.1: 解析标准化后的旧代码生成AST
      this.logStep('解析旧代码 AST', '使用 @babel/parser 解析标准化后的旧代码', { status: 'parsing' });
      const oldAst = parse(normalizedOld, this.parserOptions);
      this.logStep('解析旧代码 AST 完成', 'AST 解析成功', {
        programType: oldAst.type,
        bodyLength: oldAst.program.body.length,
      });

      // 步骤2.2: 解析标准化后的新代码生成AST
      this.logStep('解析新代码 AST', '使用 @babel/parser 解析标准化后的新代码', { status: 'parsing' });
      const newAst = parse(normalizedNew, this.parserOptions);
      ...
      const oldSigTotal = [...oldSignatures.values()].reduce((a, b) => a + b, 0);
      const newSigTotal = [...newSignatures.values()].reduce((a, b) => a + b, 0);
      astStructureDebug = {
        oldSignatureCount: oldSigTotal,
        newSignatureCount: newSigTotal,
        matchedCount: structuralResult.matchedCount,
        totalCount: structuralResult.totalCount,
      };
  ...
  private compareStructuralSignatures(
  ...
  ): { similarity: number; changes: ASTStructureChange[]; matchedCount: number; totalCount: number } {
    const changes: ASTStructureChange[] = [];
    ...
    return { similarity, changes, matchedCount, totalCount };
  }
  ...
  private extractStructure(ast: t.File): CodeStructure {
    // 收集 AST 子树的结构签名（忽略标识符和字面量的具体值）
    const collectSubtreeSignature = (node: t.Node): string => {
      const types: string[] = [];
      const visit = (n: t.Node, depth: number) => {
        if (depth > 20) return;
        // 只保留结构类型，忽略具体值
        if (t.isIdentifier(n)) {
          types.push('ID');
        } else if (t.isStringLiteral(n) || t.isNumericLiteral(n) || t.isBooleanLiteral(n)) {
          types.push('LIT');
        } else {
          types.push(n.type);
        }
        for (const key of t.VISITOR_KEYS[n.type] || []) {
          ...
          if (Array.isArray(child)) {
            child.forEach((c: any) => { if (t.isNode(c)) visit(c, depth + 1); });
    ...
    // 计算 AST 子树的节点数（代表复杂度/代码量）
    const countASTNodes = (node: t.Node): number => {
      let count = 0;
      const visit = (n: t.Node) => {
        ...
        for (const key of t.VISITOR_KEYS[n.type] || []) {
          const child = (n as any)[key];
          if (Array.isArray(child)) {
            child.forEach((c: any) => { if (t.isNode(c)) visit(c); });
          } else if (t.isNode(child)) {
            visit(child);
          }
        }
    ...
    traverse(ast, {
      ...
      FunctionDeclaration(path) {
        if (path.node.id) {
          ...
          structure.functions.push({
            ...
            signature: collectSubtreeSignature(body),
            bodySize: countASTNodes(body),
          });
        }
      },
      ...
      VariableDeclarator(path) {
        if (
          ...
          structure.functions.push({
            name: path.node.id.name,
            line: path.node.loc?.start.line || 0,
            signature: collectSubtreeSignature(funcNode.body),
            bodySize: countASTNodes(funcNode.body),
          });
      ...
      ClassDeclaration(path) {
        ...
        structure.classes.push({
          ...
          properties,
          line: path.node.loc?.start.line || 0,
          signature: collectSubtreeSignature(path.node.body),
          bodySize: countASTNodes(path.node.body),
        });
      },
   ...
   * 
   * 原理：
   * 1. 对每个函数/类提取 AST 子树的结构签名（忽略变量名和字面量值）
   * 2. 用贪婪最优匹配将两份代码的函数/类按结构相似度配对
   * 3. 每对的相似度按 LCS 系数计算（结构签名序列的最长公共子序列）
   * 4. 权重 = 该函数/类的 AST 节点数（复杂度越高权重越大，信息论依据：
   *    大函数携带更多语义信息，对整体相似度的影响应更大）
   * 5. 最终相似度 = sum(匹配对的相似度 × 复杂度) / sum(所有函数/类的复杂度)
   * 
   * 导入的权重也不是固定的，而是按导入数量占总结构元素数量的比例自然分配
   */
  private calculateSemanticSimilarity(oldCode: string, newCode: string): number {
    try {
      ...
      const importWeight = oldStructure.imports.length + newStructure.imports.length;

      // 函数匹配：按 AST 结构签名做最优匹配，权重 = 函数体节点数
      const funcResult = this.matchByStructure(
        oldStructure.functions,
        newStructure.functions
      );

      // 类匹配：同理
      const classResult = this.matchByStructure(
        oldStructure.classes,
        newStructure.classes
      );

      // 按复杂度加权合并（每个维度的权重 = 该维度的总 AST 节点数）
      const totalWeight = importWeight + funcResult.totalWeight + classResult.totalWeight;

      if (totalWeight === 0) return 100;

      const weightedSum =
        importSim * importWeight +
        funcResult.weightedSimilarity +
        classResult.weightedSimilarity;
   ...
   * 基于 AST 结构签名的最优匹配
   * 
   * 用贪婪算法将两组函数/类按结构相似度配对（不依赖名字）。
   * 每对的相似度 = 结构签名的 LCS 系数（Dice 公式）。
   * 未匹配到的项相似度为 0，但其复杂度仍计入总权重。
   * 
   * @returns weightedSimilarity: sum(匹配对相似度 × 节点数)
   *          totalWeight: 所有项的节点数之和
   */
  private matchByStructure(
    oldItems: Array<{ signature: string; bodySize: number }>,
    newItems: Array<{ signature: string; bodySize: number }>
  ): { weightedSimilarity: number; totalWeight: number } {
    if (oldItems.length === 0 && newItems.length === 0) {
      return { weightedSimilarity: 0, totalWeight: 0 };
    }

    // 计算所有对的签名 LCS 相似度
    const candidates: Array<{ sim: number; oi: number; ni: number }> = [];
    for (let oi = 0; oi < oldItems.length; oi++) {
      const oldSig = oldItems[oi].signature.split(',');
      for (let ni = 0; ni < newItems.length; ni++) {
        const newSig = newItems[ni].signature.split(',');
        const lcsLen = this.lcsLength(oldSig, newSig);
        const total = oldSig.length + newSig.length;
        const sim = total === 0 ? 1 : (2 * lcsLen) / total; // Dice 系数
        if (sim > 0.1) {
    ...
    // 贪婪匹配：按相似度降序，每个 item 只匹配一次
    candidates.sort((a, b) => b.sim - a.sim);
    const usedOld = new Set<number>();
    const usedNew = new Set<number>();
    let weightedSimilarity = 0;

    for (const { sim, oi, ni } of candidates) {
      if (usedOld.has(oi) || usedNew.has(ni)) continue;
      usedOld.add(oi);
      usedNew.add(ni);
      // 权重 = 两者复杂度的平均值
      const weight = (oldItems[oi].bodySize + newItems[ni].bodySize) / 2;
      weightedSimilarity += sim * weight;
    }

    // 未匹配项的复杂度也要计入总权重（相似度 = 0）
    const totalWeight =
      oldItems.reduce((s, item) => s + item.bodySize, 0) / 2 +
      newItems.reduce((s, item) => s + item.bodySize, 0) / 2;

    return { weightedSimilarity, totalWeight: Math.max(totalWeight, 1) };
  }
